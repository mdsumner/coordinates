...
In R there are many formal and informal methods for storing coordinates. The informal ones are efficient and fast. There's no efficient way to store raw coordinates in a data frame with reference system metadata: the closest was sp's `SpatialPointsDataFrame` but that only worked by pretending to be a data frame. The tidyverse still has no coordinates data frame mechanism. 
